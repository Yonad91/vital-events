...
const RegistrarDashboard = ({ user, setUser }) => {
  ...
  const [reportsTab, setReportsTab] = useState("prepare"); // "received" or "prepare"
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash.startsWith('#register/')) {
        ...
        if (['birth', 'marriage', 'death', 'divorce'].includes(eventType)) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/users/registrar/events${query}` , {
        token: user.token,
      });
      setEvents(Array.isArray(res) ? res : []);
    } catch (err) {
      console.error("Load registrar events error:", err);
      setEvents([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      setStats({ birth: 0, marriage: 0, death: 0, divorce: 0 });
    }
  }, [user?.token]);
  ...
  const loadReportEvents = useCallback(async () => {
    ...
    setReportError("");
    try {
      const res = await apiFetch(`/users/registrar/events`, { token: user.token });
  ...
  const handleFormSubmit = async (payload) => {
    try {
      if (editingEvent) {
        await apiFetch(`/users/registrar/events/${editingEvent._id || editingEvent.id}`, {
          method: 'PUT',
          token: user.token,
        ...
        const saved = await apiFetch('/users/registrar/events', {
          method: 'POST',
          token: user.token,
          ...(payload.isForm ? { body: payload.body, isForm: true } : { body: payload.body }),
        });
  ...
  const handleSubmitToManager = async (eventId) => {
    try {
      await apiFetch(`/users/registrar/events/${eventId}/submit`, {
        method: "PATCH",
        token: user.token,
      });
      await load();
  ...
  const handleDeleteEvent = async (eventId) => {
    ...
    try {
      ...
      const response = await apiFetch(`/users/registrar/events/${eventId}`, {
        method: "DELETE",
        token: user.token,
      });
  ...
  const handleSubmitAllToManager = async () => {
    try {
      ...
      for (const ev of drafts) {
        ...
        try {
          await apiFetch(`/users/registrar/events/${id}/submit`, { method: "PATCH", token: user.token });
        } catch (err) {
              ...
              <CardContent className="p-8">
                <div className="text-center">
                  <h2 className="text-3xl font-bold text-gray-800 mb-4">
                    {translate(lang, "Welcome to Registrar Dashboard", "ወደ ምዝገባ ዳሽቦርድ እንኳን ደህና መጡ")}
                  </h2>
                  <p className="text-lg text-gray-600 mb-6">
                    {translate(lang, "Manage vital events registration with ease. Register births, marriages, deaths, and divorces efficiently.", "የጠቃሚ ክስተቶችን በቀላሉ ያስተዳድሩ። የትውልድ፣ ጋብቻ፣ ሞት እና ፍቺ መዝገቦችን በብቃት ይመዝግቡ።")}
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="text-2xl mb-2">👶</div>
                      <h3 className="font-semibold text-blue-800">{translate(lang, "Birth Registration", "የትውልድ ምዝገብ")}</h3>
                      <div className="font-bold text-4xl text-blue-700 my-1">{stats.birth}</div>
                    </div>
                    <div className="bg-pink-50 p-4 rounded-lg">
                      <div className="text-2xl mb-2">💒</div>
                      <h3 className="font-semibold text-pink-800">{translate(lang, "Marriage Registration", "የጋብቻ ምዝገብ")}</h3>
                      <div className="font-bold text-4xl text-pink-700 my-1">{stats.marriage}</div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="text-2xl mb-2">⚰️</div>
                      <h3 className="font-semibold text-gray-800">{translate(lang, "Death Registration", "የሞት ምዝገብ")}</h3>
                      <div className="font-bold text-4xl text-gray-700 my-1">{stats.death}</div>
                    </div>
                    <div className="bg-red-50 p-4 rounded-lg">
                      <div className="text-2xl mb-2">📄</div>
                      <h3 className="font-semibold text-red-800">{translate(lang, "Divorce Registration", "የፍቺ ምዝገብ")}</h3>
                      <div className="font-bold text-4xl text-red-700 my-1">{stats.divorce}</div>
                    </div>
                  </div>
                </div>
              </CardContent>
