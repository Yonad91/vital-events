...
const MosqueDashboard = ({ user, setUser }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/users/mosque/events${query}`, {
        token: user.token,
      });
      setEvents(Array.isArray(res) ? res : []);
    } catch (err) {
      console.error("Load mosque events error:", err);
      setEvents([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      const data = await apiFetch("/users/mosque/event-stats", { token: user.token });
  ...
  const loadReportEvents = useCallback(async () => {
    ...
    setReportError("");
    try {
      const res = await apiFetch(`/users/mosque/events`, { token: user.token });
  ...
  const handleSendLetter = async () => {
    if (!letterForm.title.trim() || !letterForm.content.trim()) {
      alert(translate(lang, "Title and content are required", "ርዕስ እና ይዘት ያስፈልጋሉ"));
      return;
    }
  ...
  const handleFormSubmit = async (payload) => {
    try {
      if (editingEvent) {
        await apiFetch(`/users/mosque/events/${editingEvent._id || editingEvent.id}`, {
          method: 'PUT',
          token: user.token,
        ...
        const saved = await apiFetch('/users/mosque/events', {
          method: 'POST',
          token: user.token,
          ...(payload.isForm ? { body: payload.body, isForm: true } : { body: payload.body }),
        });
  ...
  const handleSubmitToManager = async (eventId) => {
    try {
      await apiFetch(`/users/mosque/events/${eventId}/submit`, {
        method: "PATCH",
        token: user.token,
      });
      await load();
  ...
  const handleDeleteEvent = async (eventId) => {
    ...
    try {
      ...
      const response = await apiFetch(`/users/mosque/events/${eventId}`, {
        method: "DELETE",
        token: user.token,
      });
  ...
  const handleSubmitAllToManager = async () => {
    try {
      ...
      for (const ev of drafts) {
        ...
        try {
          await apiFetch(`/users/mosque/events/${id}/submit`, { method: "PATCH", token: user.token });
        } catch (err) {
      ...
      <div className="flex-1 p-4 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">
            {translate(lang, "Mosque Dashboard", "የመስጊድ ዳሽቦርድ")}
          </h1>
              ...
              <CardContent className="p-8">
                <div className="text-center">
                  <h2 className="text-3xl font-bold text-gray-800 mb-4">
                    {translate(lang, "Welcome to Mosque Dashboard", "ወደ መስጊድ ዳሽቦርድ እንኳን ደህና መጡ")}
                  </h2>
                  <p className="text-lg text-gray-600 mb-6">
                    {translate(lang, "Manage Islamic vital events registration. Register marriages and deaths according to Islamic traditions and requirements.", "የእስላም ጠቃሚ ክስተቶችን ያስተዳድሩ። የጋብቻ እና የሞት ምዝገቦችን በእስላም ልማዶች እና መስፈርቶች ይመዝግቡ።")}
                  </p>
