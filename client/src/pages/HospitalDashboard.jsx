...
const HospitalDashboard = ({ user, setUser }) => {
  ...
  const [stats, setStats] = useState({ birth: 0, death: 0 });
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash.startsWith('#register/')) {
        ...
        if (['birth', 'death'].includes(eventType)) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/users/hospital/events${query}`, {
        token: user.token,
      });
      setEvents(Array.isArray(res) ? res : []);
    } catch (err) {
      console.error("Load hospital events error:", err);
      setEvents([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadReportEvents = useCallback(async () => {
    ...
    setReportError("");
    try {
      const res = await apiFetch(`/users/hospital/events`, { token: user.token });
  ...
  const handleFormSubmit = async (payload) => {
    try {
      if (editingEvent) {
        await apiFetch(`/users/hospital/events/${editingEvent._id || editingEvent.id}`, {
          method: 'PUT',
          token: user.token,
        ...
        const saved = await apiFetch('/users/hospital/events', {
          method: 'POST',
          token: user.token,
          ...(payload.isForm ? { body: payload.body, isForm: true } : { body: payload.body }),
        });
  ...
  const handleSubmitToManager = async (eventId) => {
    try {
      await apiFetch(`/users/hospital/events/${eventId}/submit`, {
        method: "PATCH",
        token: user.token,
      });
      await load();
  ...
  const handleDeleteEvent = async (eventId) => {
    ...
    try {
      ...
      const response = await apiFetch(`/users/hospital/events/${eventId}`, {
        method: "DELETE",
        token: user.token,
      });
  ...
  const handleSubmitAllToManager = async () => {
    try {
      ...
      for (const ev of drafts) {
        ...
        try {
          await apiFetch(`/users/hospital/events/${id}/submit`, { method: "PATCH", token: user.token });
        } catch (err) {
      ...
      <div className="flex-1 p-4 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">
            {translate(lang, "Hospital Dashboard", "የሆስፒታል ዳሽቦርድ")}
          </h1>
              ...
              <CardContent className="p-8">
                <div className="text-center">
                  <h2 className="text-3xl font-bold text-gray-800 mb-4">
                    {translate(lang, "Welcome to Hospital Dashboard", "ወደ ሆስፒታል ዳሽቦርድ እንኳን ደህና መጡ")}
                  </h2>
                  <p className="text-lg text-gray-600 mb-6">
                    {translate(lang, "Manage medical vital events registration. Register births and deaths with medical accuracy and efficiency.", "የሕክምና ጠቃሚ ክስተቶችን ያስተዳድሩ። የትውልድ እና የሞት ምዝገቦችን በሕክምና ትክክለኛነት እና ብቃት ይመዝግቡ።")}
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-blue-50 p-6 rounded-lg">
                      <div className="text-3xl mb-3">👶</div>
                      <h3 className="font-semibold text-blue-800 text-lg">{translate(lang, "Birth Registration", "የትውልድ ምዝገብ")}</h3>
